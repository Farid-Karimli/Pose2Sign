...
# SLF_ROPE: RoPE for Self-Attention with all frames
@torch.cuda.amp.autocast(enabled=False)
def rope_apply_with_alllens(
    x,                    # [B, S, N, D]
    grid_sizes,           # [B, 3]  (f, h, w)
    freqs,                # [S_max, D/2]  complex frequencies
    motion_scale=None,    # nn.Parameter / Tensor / float; if None → 0
    space_scale_factor=None,  # nn.Parameter / Tensor / float; if None → 0.02
    ...
    # Split freq(t,h,w)
    ct, ch, cw = c - 2 * (c // 3), c // 3, c // 3
    freqs_t, freqs_h_all, freqs_w_all = freqs.split([ct, ch, cw], dim=1)
    # -----------------------------------

    out_list = []
    for b, (f, h, w) in enumerate(grid_sizes.tolist()):
        seq_len = f * h * w

        # 2.1 token to 复数
        # 2.1 token to complex
        x_b = torch.view_as_complex(
            x[b, :seq_len].to(torch.float32).reshape(seq_len, N, -1, 2)
        )

        # 2.2 frequency tensors (expanded to grid)
        f_t = freqs_t[:f].view(f, 1, 1, -1).expand(f, h, w, -1)
        f_h = freqs_h_all[:h].view(1, h, 1, -1).expand(f, h, w, -1)
        f_w = freqs_w_all[:w].view(1, 1, w, -1).expand(f, h, w, -1)

  
        Ch, Cw = f_h.size(-1), f_w.size(-1)
        low_h = slice(int(Ch * (1 - low_ratio)), Ch)
        low_w = slice(int(Cw * (1 - low_ratio)), Cw)

        if space_scaling != 1.0: # only clone when scaling is needed
            f_h = f_h.clone()
            f_w = f_w.clone()
            f_h[..., low_h] *= space_scaling
            f_w[..., low_w] *= space_scaling

        # 3.1 concatenate & apply rotation
        freqs_b = torch.cat([f_t, f_h, f_w], dim=-1).reshape(seq_len, 1, -1)
        x_rot   = torch.view_as_real(x_b * freqs_b).flatten(2)
        out_b   = torch.cat([x_rot, x[b, seq_len:]])
...
class WanSelfAttention(nn.Module):
    ...
    def __init__(self,
        ...
        self.motion_scale = nn.Parameter(torch.ones(1) * 1.5) #可学习的运动缩放因子, learnable motion scaling factor
        self.space_scale_factor = nn.Parameter(torch.ones(1) * 0.02)  # 可学习的空间缩放因子,leanable space scaling factor
