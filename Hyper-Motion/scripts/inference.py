...
from hyper.utils.utils import (filter_kwargs, get_image_to_video_latent,
...
# Other params
sample_size         = [576, 1024] # horizontal screen 720P
video_length        = 49
...
control_video           = os.path.join(CONTROL_VIDEOS_PATH, VIDEO_FILE_NAME + ".mp4")
ref_image               = os.path.join(REF_FRAMES_PATH, VIDEO_FILE_NAME + ".png")

print(f"control_video: {control_video}, ref_image: {ref_image}")
print(f"Paths exist: {os.path.exists(control_video)}, {os.path.exists(ref_image)}")
...
def save_results():
    ...
    if video_length == 1:
        ...
        save_videos_grid(sample_without_first_frame, video_path, fps=fps)
