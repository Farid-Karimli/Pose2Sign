...
if __name__ == "__main__":
    ...
    controlnext_state = OrderedDict()
    for name, data in state_dict.items():
        model = name.split('.', 1)[0]
        module = name.split('.', 1)[1]
        if model == 'unet':
            unet_state[module] = data
        elif model == 'controlnext':
            controlnext_state[module] = data
    ...
    torch.save(controlnext_state, os.path.join(args.checkpoint_dir, "controlnext", "diffusion_pytorch_model.bin"))
