...
log_path = os.path.join("logs", f"train_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
...
def main():
    ...
    train_dataset = ASLVideoDataset(data_folder_path=args.train_data_path)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(controlnet):
                ...
                loss = loss.mean()
                log_file.write(f"Epoch {epoch}, Step {step}: loss = {loss:.6f}\n")
